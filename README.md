...
- **A deterministic conformance check that replaced a vendor review cycle.** An approved source document and the digital version built from it go in; either a certificate of conformance comes out, or a located finding does. Same inputs, byte-identical output. A difference matching no ruling blocks the certificate rather than passing, and the tool has no mechanism to rule on one — quality owns the ruling set, the code owns nothing but detection. Establishing that a standard existed was harder than the comparison: a review of several thousand recorded findings across hundreds of documents found that no class of difference had ever blocked a document, so the review set a standard for the first time rather than confirming one. Built with an AI coding agent.
